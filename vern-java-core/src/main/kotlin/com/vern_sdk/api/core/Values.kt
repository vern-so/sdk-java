...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { VernInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw VernInvalidDataException("`$name` is not set")
            is JsonNull -> throw VernInvalidDataException("`$name` is null")
            else -> throw VernInvalidDataException("`$name` is invalid, received $this")
        }
